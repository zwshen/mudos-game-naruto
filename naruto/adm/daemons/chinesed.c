...
string chinese_period(int t)
{
    ...
    if(y)
        time = chinese_number(y) + "年又";
    if(n)
        time += chinese_number(n) + "個月"    + (strlen(time)?"":"又");
    if(d)
        time += chinese_number(d) + "天"        + (strlen(time)?"":"又");
    if(h)
        time += chinese_number(h) + "小時"    + (strlen(time)?"":"又");
    if(m)
        time += chinese_number(m) + "分"        + (strlen(time)?"":"又");
