...
varargs mixed do_emote(object me, string verb, string arg, int channel_emote, int rumor)
{
    ...
    if( stringp(str = emote[verb]["myself" + msg_postfix]) ) {
        if( rumor ) str = replace_string(str, "$N", me->name() + "(rumor)" );
        else str = replace_string(str, "$N", me->name() + "(" + me->query_id() + ")" );
        str = replace_string(str, "$P", gender_self(my_gender));
        if( objectp(target) ) {
            str = replace_string(str, "$n", target->name() + "(" + target->query_id() + ")" );
            str = replace_string(str, "$p", gender_pronoun(target_gender));
        }
        //        if( !channel_emote ) message("emote:vision", CYN + str + NOR, me);
        if( !channel_emote ) {
            if( environment(me)->is_area() ) write( CYN + str + NOR );
            else message("emote:vision", CYN + str + NOR, me);
        }
    }
    ...
    if( objectp(target) && stringp(str = emote[verb]["target"]) ) {
        ...
        if( !channel_emote ) {
            if( environment(me)->is_area() ) tell_object(target, CYN + str + NOR );
            else message("emote:vision", CYN + str + NOR, target);
        }
    }
    ...
    if( stringp(str = emote[verb]["others"+msg_postfix]) ) {
        ...
        if( !channel_emote ) {
            if( environment(me)->is_area() ) tell_area(environment(me), info["x_axis"], info["y_axis"], CYN + str + NOR, ({me, target}) );
            else message("emote:vision", CYN + str + NOR, environment(me), ({ me, target }));
        }
        else return str;
    }
    if( channel_emote ) return 0;
