...
//    UPTIME_CMD->main();
    ...
    printf("獵火影忍者人已經執行了" + CHINESE_D->chinese_period(uptime()) + "。\n");
...
confirm_password(string pass, object ob)
{
    ...
    write(@TEXT
為了避免您的人物遭人盜用，Naruto 將採用 email 認證方式保護
人物所有權，所以請您輸入一個可供接收認證用電子郵件的地址。
...
//    string startroom, err, mot;
   		...
   		!area_move(room, user, user->query("area_info/x_axis"), user->query("area_info/y_axis") ) )
    {
    	...
    	if( !room || !user->move(room) ) {
    	    ...
    	    if( !room || !user->move(room) ) {
        	    write("對不起，目前系統正在整修一些登入地點的程式，請稍候再試。\n");
			    destruct(ob);
			    destruct(user);
	 	    }
	 	}
...
{
    CHANNEL_D->do_channel( this_object(), "sys", sprintf("%s(%s)斷線了。", ob->name(1), ob->query("id")));
    ob->move("/obj/void");
}
...
int check_legal_id(string id)
{
    ...
    if( (strlen(id) < 3) || (strlen(id) > 10 ) ) {
        write("對不起﹐您的英文名字必須是 3 到 10 個英文字母。\n");
        return 0;
    }
    while(i--)
        if( id[i]<'a' || id[i]>'z' ) {
