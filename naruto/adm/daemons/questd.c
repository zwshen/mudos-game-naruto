...
private void insertAssigner(string npc_file, string quest_file)
{
...
private void insertRewarder(string npc_file, string quest_file)
{
...
void doScanQuest(string dir)
{
    ...
    if( !stringp(dir) ) dir = "/world/quest/";

    files = get_dir(dir, -1);

    if( !sizeof(files) ) {
        if( file_size(dir) == -2 ) write("QUESTD: 任務目錄是空的。 (" + dir + ")\n");
        else write("QUESTD: 沒有這個任務目錄。 (" + dir + ")\n");
        return;
    }
...
private void confirmAssign(string input, object player, string quest_file)
{
if( input != "" ) {
...
private void doAssignQuest(object npc, object player, string quest_file)
{
    int i, message_size;
...
// 取得可解的任務index值
private int* getQuestIndex(object player, string npc_file)
{
    int i;
    string quest_file;
    int *index = ({});
...
private void getSelect(string input, object player, object npc, int *index)
{
int select;
...
private int checkKill(object npc, object player, string quest_file)
{
    int i, size, *value;
string *key;
    mapping kill;
...
private int checkItem(object npc, object player, string quest_file)
{
...
//            write("    身上物品：" + inv[j]->short() + " (" + item_file + ")\n");
            ...
            message_vision("$N對著$n說道：看來你已經帶來了" + inv[j]->name() + "。\n", npc, player);
...
private int isReward(object npc, object player, string quest_file)
{
    int i, message_size;
