...
private void create()
{
    ...
    foreach(entry in wizlist)
    {
        if( entry[0]=='#'
            ...
            continue;
        if( member_array(lvl, wiz_levels)==-1 )
            continue;
        wiz_status[uid] = lvl;
    }
}
...
int valid_write(string file, object user, string func)
{
    ...
    if( sizeof(path)>=3 && euid[0]>='a' && euid[0]<='z'
            && path[0]=="u" && path[1]==euid[0..0] && path[2]==euid )
        return 1;

    /* 允許 SIMUL_EFUN_OB 寫入 /log 檔 */
    if( file==SIMUL_EFUN_OB && path[0]=="log" )
        return 1;
    ...
    for( i=sizeof(path)-1; i>=0; i--)
    {
        ...
        if( undefinedp(perm = exclude_write[dir]) )
            continue;
    ...
    {
        dir = "/" + implode(path[0..i], "/");
        if( undefinedp(perm = trusted_write[dir]) )
            continue;
        if( (member_array(euid, perm)>=0) || (member_array(status, perm)>=0) )
            return 1;
    }
...
int valid_read(string file, mixed user, string func)
{
    ...
    {
        dir = "/" + implode(path[0..i], "/");
        if( undefinedp(perm = exclude_read[dir]) )
            continue;
        if( (member_array(euid, perm)>=0) || (member_array(status, perm)>=0) )
            return 0;
    }
    ...
    {
        dir = "/" + implode(path[0..i], "/");
        if( undefinedp(perm = trusted_read[dir]) )
            continue;
        if( (member_array(euid, perm)>=0) || (member_array(status, perm)>=0) )
            return 1;
    }
