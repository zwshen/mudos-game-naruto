...
int main(object me, string arg)
{
    ...
    // add by dragoon
    if( objectp(who) && (who->query("no_fight") && !present("_MY_PET_", environment(me))) )
...
private int do_give(object me, object item, object who)
{
    ...
    if( item->query("keep_this") )
    	return notify_fail("這樣東西你正 keep 著。\n");

    if( item->query("equipped") )
    	return notify_fail("這樣東西你正裝備著。\n");
...
//    if( wizardp(me) && (userp(who) && !wizardp(who)) )
    ...
    message_vision("$N交給$n" + (item->query_amount() ? item->query_amount() : "一") + item->query("unit") + item->name() + "。\n", me, who);
