...
return notify_fail("這樣東西無法被移動。\n");
    ...
    if( obj->query("keep_this") ) 
        return notify_fail("這樣東西你已經 keep 無法被移動。\n");
    
    if( obj->query("equipped") )
        return notify_fail("這樣東西你已經裝備著無法被移動。\n");
...
*/
if( !function_exists("query_amount", obj) ) amount = 1;
    else amount = obj->query_amount();

    if( obj->move(dest) )
        message_vision( "$N將" + chinese_number(amount) + obj->query("unit") + obj->name() + "放進$n。\n", me, dest);
