...
mapping query_apprentices()
{
    return masters;
}

int is_master_of(object ob)
{
    return !undefinedp(apprentices[IDENTITY(ob)]);
}
int is_apprentice_of(object ob)
{
    return !undefinedp(masters[IDENTITY(ob)]);
}
...
// initialization.
int init_apprentice(object ob)
{
    return add_apprentice(ob);
}
