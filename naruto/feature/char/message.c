...
void receive_message(string msgclass, string msg)
{
    ...
    case "channel":
        if( !objectp(link = this_object()->link()) ||
                !pointerp(ch = link->query("channels")) || member_array(msgclass, ch)==-1 )
            return;
        break;
    case "outdoor":
        ...
        break;
    default:
        if( this_player() && (this_player() != this_object()) && !this_player()->visible(this_object()) )
            return;
    }
...
private void write_prompt()
{
    if( sizeof(msg_buffer) )
    {
        ...
        foreach(string msg in msg_buffer) receive(msg);
        msg_buffer = ({});
    }
    receive("> ");
}

private void receive_snoop(string msg)
{
    receive("※" + msg);
}
