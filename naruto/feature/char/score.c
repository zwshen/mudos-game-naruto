...
varargs void gain_score(string course, int xp)
{
    ...
    add("score/" + course, xp);
    if( undefinedp(score_gain[course]) )
        score_gain[course] = xp;
    else
        score_gain[course] += xp;
...
/* 升級不看這個了 by Acme
    ...
    if( !mapp(sc) || !mapp(targ_sc) )
        return;

    foreach(s, v in targ_sc) if( sc[s] < v )
        return;
...
{
    if( !query("score/" + course) )
        set
            ("score/" + course, 0);
    if( xp < (int)query("target_score/" + course) )
        return;
    set
        ("target_score/" + course, xp);
}
