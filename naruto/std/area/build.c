...
{
    if( undefinedp(builds[file]) ) return 0;
    else return 1;
}
...
{
    if( !isBuildExist(file) ) return 0;
    if( !mapp(builds[file]) ) return 0;
    return builds[file];
}
...
void setBuildTitle(string file, int index, string title)
{
    ...
    if( !isBuildExist(file) ) return;
    title = color(title);
    if( strlen(uncolor(title)) > 2 ) return;

    buildFile = BUILD_FILE_PATH + builds[file]["style"] + ".c";
    if( index <= 0 ) return;
    if( index > buildFile->getTitleLength() ) return;
    
    temp = buildFile->getTitlePosition(index);
    if( sscanf(temp, "(%d,%d)", shiftX, shiftY) != 2 ) return;
...
string getBuildFolderPath()
{
    ...
    if( !(path = getAreaFolderPath()) ) return 0;
...
{
    string buildFile;
    if( !(buildFile = this_object()->query_data(x, y, "_BUILDING_FILE_")) ) return 0;
    if( undefinedp(builds[buildFile]) ) return 0;
    return buildFile;
}
...
private int isBuilding(int x, int y, string *checkPoints)
{
    ...
    for(i=0;i<yMax;i++) {
        ...
        for(j=0;j<xMax;j++) {
            if( checkPoints[i][j..j] == "0" ) continue;

            // 沒有設 _BUILDING_
            if( !this_object()->query_data(x+j, y+i, "_BUILDING_") ) return 0;

            // Block
            if( this_object()->query_data(x+j, y+i, "block") ) return 0;
            
            // 被佔領
            if( hasBuilding(x+j, y+i) ) return 0;
        }
    }
...
private void setWall(int x, int y, string *wall, int on)
{
    ...
    for(i=0;i<yMax;i++) {
        xMax = strlen(wall[i]);
        for(j=0;j<xMax;j++) {
            if( wall[i][j..j] == "0" ) continue;
            if( !on )
                this_object()->delete_data(x+j, y+i, "block");
            else this_object()->set_data(x+j, y+i, "block", on);
        }
    }
}
...
private object createRoomBuilding(int x, int y, string buildFile, string filePath, string fileName)
{
    ...
    // write file
    if( !write_file(filePath + fileName, code) ) return 0;
    if( !objectp(build = load_object(filePath + fileName)) ) return 0;
    ...
    for(i=0;i<exits_size;i++) {
        ...
        if( sscanf(temp, "(%d,%d)", exitX, exitY) != 2 ) continue;
...
private object createAreaBuilding(int x, int y, string buildFile, string filePath, string fileName)
{
    ...
    if( sscanf(temp, "(%d,%d)", exitX, exitY) != 2 ) return 0;
    ...
    for(i=0;i<exits_size;i++) {
        temp = exits_key[i];
        if( sscanf(temp, "(%d,%d)", entryX, entryY) != 2 ) continue;
...
int createBuilding(string style, string owner, int x, int y)
{
    ...
    if( file_size(buildFile) == -1 ) return 0;

    // 是一個area build
    if( this_object()->isBuild() && buildFile->isAreaBuilding() ) return 0;

    // 檢查能否建立build
    temp = buildFile->getCheckPointsPosition();
    if( sscanf(temp, "(%d,%d)", checkX, checkY) != 2 ) return 0;
    checkX += x;
    checkY += y;
    if( !isBuilding(checkX, checkY, buildFile->getCheckPoints()) ) return 0;

    // get file path
    if( !(filePath = getBuildFolderPath()) ) return 0;
...
int destBuilding(int x, int y)
{
    ...
    // 沒被佔領
    if( !hasBuilding(x, y) ) return 0;
    
    if( !(fileName = getBuildFile(x, y)) ) return 0;
