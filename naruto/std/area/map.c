...
string *query_exits(int x, int y, int option)
{
    ...
    if( !option ) {
        ...
        if( y-1 >= 0 ) {
            ...
            if( x+1 < sizeof(area[0]) && is_move(x+1, y-1) ) exits += ({ "東北("HIK"ne"NOR")" });
        }
        ...
        if( y+1 < sizeof(area) ) {
            ...
            if( x+1 < sizeof(area[0]) && is_move(x+1, y+1) ) exits += ({ "東南("HIK"se"NOR")" });
        }

        // 往東可能有路
        if( x+1 < sizeof(area[0]) && is_move(x+1, y) ) exits += ({ "東("HIK"e"NOR")" });
...
varargs string show_objects(int x, int y, int type)
{
    ...
    if( sizeof(area[y][x]["objects"]) >= 30 ) return "這裡的東西太多，一時看不清楚...\n";
...
varargs int map_ansi_save()
{
    ...
    if( x <= 49 ) {
        ...
        else write("儲存右半邊ANSI檔成功\(" + file + "_right.ansi)。\n");
    }
