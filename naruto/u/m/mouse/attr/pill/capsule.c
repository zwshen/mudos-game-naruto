...
void create()
{
    set_name(RED"重"HIW"生"HIK"膠囊"NOR, ({ "capsule" }));
    switch( random(4) ) {
        case 0: set("name",HIR"[力量]"NOR+query("name"));
                set("attr","str");
                break;
        case 1: set("name",YEL"[體格]"NOR+query("name"));
                set("attr","con");
                break;
        case 2: set("name",HIC"[敏捷]"NOR+query("name"));
                set("attr","dex");
                break;
        case 3: set("name",HIB"[智慧]"NOR+query("name"));
                set("attr","int");
                break;
    }
    set("unit", "粒");
