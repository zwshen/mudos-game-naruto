...
void do_fight()
{
    ...
    message_vision("$N將『"CYN"水遁"NOR"』之印結於掌心，包住嘴巴吐出「"HIC"水連彈"NOR"」\n", this_object(), enemy);
    damage = random(30);
    if( random(10) < random(2)) {
    message_vision("        $n縱身跳開了水彈的攻擊範圍！\n", this_object(), enemy);
    } else if( damage < 1) {
    message_vision("        $n被水彈打中，除了身子晃了一下並無大礙。(0)"+report_status(enemy)+"", this_object(), enemy);
    } else {
    enemy->damage_stat("hp", damage, enemy);
    message_vision("        $n被水彈擊中胸口，頓時感覺一陣噁心！("+damage+")"+report_status(enemy)+"", this_object(), enemy);
    damage = random(40);
    }
