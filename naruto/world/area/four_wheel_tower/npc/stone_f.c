...
void do_startDamage()
{
    ...
    for(j=0;j<sizeof(pattern);j++) {
        if( pattern[j] == "/world/area_pattern/mist900_s_01.c" ) break;
    }
    for(i=0; i<size; i++) {
        ...
        message_vision("$N"HIR"一陣搖晃後放出大量紅光！四周景色逐漸變化...\n"NOR, this_object());
        return;
    }
    delete("change_map");
    return;
...
void create()
{
    set_name(HIW"鎮火石"NOR, ({ "stone", "fire" }) );
    set("channel_id", HIW"鎮火石"NOR);
    set_level(1);
    set_race("human");
    set("age", 1);
    set("long", "白色的巨大石頭，好像對這片環境的高溫完全不受影響，不動如山。\n"
                "偶爾還會發出紅色的閃光，但是石頭表面卻仍然保持白色。\n");
    set("chat_chance", 1);
    set("chat_msg", ({
        (: do_startDamage :),
    }));
