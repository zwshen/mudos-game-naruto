...
int do_down(string arg)
{
    int x, y;
    object me, area;
    string area_file = "/world/area/four_wheel_tower/sand_2f.c";
...
int do_up(string arg)
{
    int x, y;
    object me, area, npc;
    string area_file = "/world/area/four_wheel_tower/sand_4f.c";
    ...
    if( !objectp(area = load_object(area_file)) ) return 0;
    if( !area->is_area() ) return 0;
    if( npc = present("_KING_THREE_", environment(me)) ) {
        message_vision("通往四樓的路被$N擋住了，$n無法通過。\n", npc, me);
        return 1;
    }
...
int valid_leave(object me, string dir)
{
    ...
    if( userp(me) && ob->query_data(x, y, "check_robot") ) {
        ...
        if( !objectp(item) ) {
            ...
            tell_object(this_player(), YEL"忽然沙堆中噴出一道金光！\n"NOR);
