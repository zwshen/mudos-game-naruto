...
{
    object me, area;
    string area_file = "/world/area/benthal/sea3.c";
    ...
    if( me->query("area_info/x_axis") != 24 || me->query("area_info/y_axis") != 24 ) return 0;

    if( !objectp(area = load_object(area_file)) ) return 0;
    if( !area->is_area() ) return 0;
    message_vision("$N高高地跳起，空翻兩圈之後落入水中。\n", me);
    if( !area_move(area, me, 27, 24) ) return 0;
    message_vision("$N從海面沉了下來。\n", me);
    return 1;
}
