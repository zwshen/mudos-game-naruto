...
void do_fight()
{
    ...
    switch( this_object()->query("sktree") ) {
        ...
        case 5: // 血球再生
            ...
            command("perform blood-cell revive.revive on " + this_object()->query_id());
            break;

        case 6: // 聖療之手
            if( this_object()->query_stat("hp") < 301 ) {
            command("perform holy hand.healing on " + this_object()->query_id());
            }
            break;
    }
}
void create()
{
    ...
    // 目前有攻擊敘述的sk裝在基礎會導致攻擊敘述出不來, 要刪掉
    // 手動perform也不需裝在enable就可使用
    switch( query("sktree") ) {
        case 1: // 肉體強化
