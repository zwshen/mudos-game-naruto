...
int accept_object(object me, object ob)
{
    ...
    if( me->query_point("learn") > 2500000 ) {
        message_vision("$N指著身後堆滿的樹液說道："HIG"樹液好像太多了，先暫時不收啦！\n"NOR, this_object());
        return 0;
    }
    me->start_busy(1);
    ...
    switch( give_check(ob) ) {
        case 1:
            if( me->query_temp("quest/helpwater") == 2) {
                ...
                tell_object(me, "(你的學習點數增加 " + ob->query_amount() +" 點了。)\n");
                me->add_point("learn", ob->query_amount() );
                do_chat( ({ ( : destruct , ob : ), }) );
                break;
            }
