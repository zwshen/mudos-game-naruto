...
void create()
{
    set_name("建築教師", ({ "architecture teacher", "teacher" }));
    set_race("human");
    set_level(50);
    set("age", random(50)+18);

    set("long", "一位可以教你如何學習建築學技能的教師。\n"
                "學習建築學技能： learn architecture\n"
...
int do_learn(string arg)
{
    ...
    if( me->query_level() < 40 )
        return notify_fail("建築學需要40級才能學習。\n");

    sk = me->getProSkill("architecture");

    // 從沒學過
    if( !mapp(sk) ) {
        message_vision("$N向$n討教有關建築學方面的知識。\n", me, this_object());
        me->addProSkill("architecture");
    } else tell_object(me, "你已經學會建築學技能了。\n");
